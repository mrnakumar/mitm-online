...
sync_interval = (
    int(os.environ.get("sync_interval"))
    if os.environ.get("sync_interval") is not None
    else 10
)
    ...
    def request(self, flow):
        if next(filter(lambda url: url in flow.request.pretty_url, self.blocked), None):
            flow.response = mitm_http.Response.make(
                404, b"How about studying", {"Content-Type": "text/html"}
            )
        else:
    ...
    def test_db_read_write(self):
        ...
        try:
            ...
            self.assertListEqual(
                list(map(lambda r: r[0], result)), ["youtube.com", "twitter.com"]
            )
        finally:
            db.close()
...
if mode_prod and db_name == ":memory:":
    ctx.log.info(
        "Must set environment variable 'db_name' and it should not be set to ':memory:' for 'prod' mode"
    )
    should_exit = True
if should_exit:
    sys.exit("Missing environment variable for DB")

addons = [Blocker(mode_prod) if mode_prod else None]
